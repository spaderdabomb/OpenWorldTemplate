...
namespace AmplifyShaderEditor
{
	...
	{
		private string m_functionHeaderSphereFrag = "SphereImpostorFragment( o, positionCS, positionWS, {0}, {1} )";
		private string m_functionHeaderSphere = "SphereImpostorVertex( {0}, {1}, {2}, {3} )";
		private string m_functionHeaderFrag = "OctaImpostorFragment( o, positionCS, positionWS, {0}, {1}, {2}, {3}, {4} )";
		...
		void UpdateTag()
		{
			if ( m_containerGraph.CurrentMasterNode != null )
			{
		...
		public override string GenerateShaderForOutput( int outputId, ref MasterNodeDataCollector dataCollector, bool ignoreLocalvar )
		{
			...
			string positionOS = dataCollector.IsSRP ? ".positionOS" : ".vertex";
			string normalOS = dataCollector.IsSRP ? ".normalOS" : ".normal";

			if( m_customImpostorType == CustomImpostorType.Spherical )
				functionResult = dataCollector.AddFunctions( m_functionHeaderSphere, m_functionBody, vertIN + positionOS, vertIN + normalOS, vertOut + "." + sphereFrames, vertOut + "." + viewPosName );
			else
				functionResult = dataCollector.AddFunctions( m_functionHeader, m_functionBody, vertIN + positionOS, vertIN + normalOS, vertOut + "." + uvFrame1Name, vertOut + "." + uvFrame2Name, vertOut + "." + uvFrame3Name, vertOut + "." + octaFrameName, vertOut + "." + viewPosName );
			...
			if( dataCollector.IsFragmentCategory )
			{
				...
				switch( outputId )
				{
					...
					return "o.Alpha";
					case 7:
					return isHDRP ? "GetAbsolutePositionWS( positionWS )" : "positionWS";
					case 16:
					dataCollector.AddLocalVariable( UniqueId, "float3 viewPosOut" + OutputId + " = mul( UNITY_MATRIX_V, float4( positionWS.xyz, 1.0 ) ).xyz;" );
			...
			else
			{
				...
				{
					case 7:
					string positionWS = "mul( UNITY_MATRIX_I_V, float4( " + vertOut + "." + viewPosName + ".xyz, 1.0 ) ).xyz";
		...
		{
			m_functionBody = string.Empty;
			IOUtils.AddFunctionHeader( ref m_functionBody, "inline void RayPlaneIntersectionUV( float3 normalOS, float3 rayPosition, float3 rayDirection, inout float2 uvs, inout float3 localNormal )" );
		...
		{
			m_functionBody = string.Empty;
			IOUtils.AddFunctionHeader( ref m_functionBody, "inline void SphereImpostorVertex( inout float4 positionOS, inout float3 normalOS, inout float4 frameUVs, inout float4 viewPos )" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "positionOS.xyz = billboard;" );
			IOUtils.AddFunctionLine( ref m_functionBody, "normalOS.xyz = objectCameraDirection;" );
			IOUtils.CloseFunctionBody( ref m_functionBody );
		}
		...
		private void GenerateSphereImpostorFragment( ref MasterNodeDataCollector dataCollector, bool withSpec = true, bool withEmission = true, bool isSRP = false )
		{
			...
			IOUtils.AddFunctionHeader( ref m_functionBody, "inline void SphereImpostorFragment( inout "+ structure + " o, out float4 positionCS, out float3 positionWS, float4 frameUV, float4 viewPos" + extraHeader + " )" );
		...
		/// <summary>
		/// Octahedron positionOS function
		/// </summary>
		/// <param name="isSRP"></param>
		private void GenerateImpostorVertex( bool isSRP = false )
		{
			m_functionBody = string.Empty;
			IOUtils.AddFunctionHeader( ref m_functionBody, "inline void OctaImpostorVertex( inout float4 positionOS, inout float3 normalOS, inout float4 uvsFrame1, inout float4 uvsFrame2, inout float4 uvsFrame3, inout float4 octaFrame, inout float4 viewPos )" );
			IOUtils.AddFunctionLine( ref m_functionBody, "float2 uvOffset = _AI_SizeOffset.zw;" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "positionOS.xyz = billboard + "+ m_offsetProp.PropertyName + ".xyz;" );
			IOUtils.AddFunctionLine( ref m_functionBody, "normalOS.xyz = objectCameraDirection;" );
		...
		void GenerateImpostorFragment( ref MasterNodeDataCollector dataCollector, bool withSpec = true, bool withEmission = true, bool isSRP = false )
		{
			...
			IOUtils.AddFunctionHeader( ref m_functionBody, "inline void OctaImpostorFragment( inout "+ structure + " o, out float4 positionCS, out float3 positionWS, float4 uvsFrame1, float4 uvsFrame2, float4 uvsFrame3, float4 octaFrame, float4 interpViewPos" + extraHeader + " )" );
			...
			bool patchDepthOffset = ( !isSRP || ASEPackageManagerHelper.PackageSRPVersion < 140000 );
			if ( patchDepthOffset )
			{
				IOUtils.AddFunctionLine( ref m_functionBody, "#if ( defined(SHADERPASS) && (defined(SHADERPASS_DEPTHNORMALSONLY) && SHADERPASS == SHADERPASS_DEPTHNORMALSONLY) ) || defined(UNITY_PASS_SHADOWCASTER)" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody , "positionWS = mul( UNITY_MATRIX_I_V, float4( viewPos.xyz, 1 ) ).xyz;" );
			IOUtils.AddFunctionLine( ref m_functionBody, "positionCS = mul( UNITY_MATRIX_P, float4( viewPos, 1 ) );" );
			if( !isSRP )
			{
				IOUtils.AddFunctionLine( ref m_functionBody, "#if defined(SHADOWS_DEPTH)" );
				IOUtils.AddFunctionLine( ref m_functionBody, "positionCS = UnityApplyLinearShadowBias( positionCS );" );
